    ...
    <AppLayout breadcrumbs={[{ label: "Dashboard", href: "/dashboard" }]}>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card className="p-5 border border-slate-200 dark:border-slate-700">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold flex items-center">
              <Clock className="w-5 h-5 mr-2 text-violet-500" /> Upcoming Tasks
            </h2>
            <Button
              variant="ghost"
              size="sm"
            ...
            {displayUpcomingTasks.map((task) => (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400">Due: {task.due}</p>
                </div>
                <Badge
                  className={
                    task.priority === "High"
          ...
          <Button className="w-full mt-4 bg-violet-600 hover:bg-violet-700 text-white">Add New Task</Button>
        </Card>
      </div>
    </AppLayout>
  )
